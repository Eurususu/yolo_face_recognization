...
from face_onnx import FaceONNX
import numpy as np
import faiss
...
def detect(opt):
    ...
    if not opt.save_feature:
        for name in os.listdir(feature_data_dir):
            if name not in feature_dict.keys():
                feature_dict[name] = []
            name_path = os.path.join(feature_data_dir, name)
            for feature_path in os.listdir(name_path):
                path = os.path.join(name_path, feature_path)
                feature_dict[name].append(np.load(path))
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print("detect time is: %f"%(time.time()-t))
        # Apply Classifier
        if classify:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                    ...
                    if webcam:
                        clip_coords(xyxy, im0s[0].shape)
                    else:
                        clip_coords(xyxy, im0s.shape)
                    # cv2.imshow('out', crop)
                    # cv2.waitKey(10)
                    mask = np.ones(len(det[det_index, 6:]), dtype=bool)
                    mask[2::3] = False
                    kps = det[det_index, 6:][mask]
                    kps = kps.reshape(5, 2).cpu().numpy()
                    t1 = time.time()
                    if webcam:
                        feat = rec.get(im0s[0], kps)
                    else:
                        feat = rec.get(im0s, kps)
                    t2 = time.time()
                    ...
                    if opt.save_feature:
                        ...
                        if not os.path.exists(file_path):
                            ...
                            print(f"Feature saved as {file_path}")
                            count += 1
                    else:
                        all_features = []
                        labels = []
                        ...
                        # 创建 Faiss 索引
                        index = faiss.IndexFlatIP(512)  # Inner Product (dot product)
                        index.add(all_features)

                        # 新特征向量
                        new_feature = feat / np.linalg.norm(feat)

                        # 搜索最相似的特征
                        D, I = index.search(new_feature.reshape(1, -1), 1)  # k=1
                        most_similar_index = I[0][0]
                        most_similar_person = labels[most_similar_index]
                        # print(most_similar_person)
                        sim = D[0][0]
                        # for data in feature_data:
                        #     tmp = rec.compute_sim(feat, data)
                        #     if sim < tmp:
                        #         sim = tmp
                        if sim < 0.32:
                            print("相似度为：%f" % sim)
                            cv2.putText(im0, "unknown", (c1[0], c1[1] - 2),  cv2.FONT_HERSHEY_SIMPLEX, 1.5,[225, 255, 255], 1, cv2.LINE_AA)
                        else:
                            print("相似度为：%f"%sim)
                            cv2.putText(im0, most_similar_person, (c1[0], c1[1] - 2), cv2.FONT_HERSHEY_SIMPLEX, 1.5, [225, 255, 255], 1,
                                        cv2.LINE_AA)
